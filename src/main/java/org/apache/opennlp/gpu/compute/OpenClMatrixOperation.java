...
public class OpenClMatrixOperation implements MatrixOperation {
    ...
    public void multiply(float[] a, float[] b, float[] c, int rowsA, int colsB, int sharedDim) {
        ...
        try {
            // Get kernel
            cl_kernel kernel = resourceManager.getOrCreateKernel(
                    "matrixMultiply", MATRIX_MULTIPLY_KERNEL);
            // Allocate buffers
            cl_mem aBuffer = resourceManager.allocateBuffer(rowsA * sharedDim * Sizeof.cl_float, "float");
            cl_mem bBuffer = resourceManager.allocateBuffer(sharedDim * colsB * Sizeof.cl_float, "float");
            cl_mem cBuffer = resourceManager.allocateBuffer(rowsA * colsB * Sizeof.cl_float, "float");
            // Copy data to device
    ...
    public void add(float[] a, float[] b, float[] c, int size) {
        log.debug("OpenCL matrix add: {} elements", size);
        try {
            // Get kernel
            cl_kernel kernel = resourceManager.getOrCreateKernel(
                    "matrixAdd", MATRIX_ADD_KERNEL);
            // Allocate buffers
            cl_mem aBuffer = resourceManager.allocateBuffer(size * Sizeof.cl_float, "float");
            cl_mem bBuffer = resourceManager.allocateBuffer(size * Sizeof.cl_float, "float");
            cl_mem cBuffer = resourceManager.allocateBuffer(size * Sizeof.cl_float, "float");
            // Copy data to device
            CL.clEnqueueWriteBuffer(
                    (cl_command_queue) resourceManager.getCachedData("commandQueue"),
                    aBuffer, CL.CL_TRUE, 0, size * Sizeof.cl_float,
                    Pointer.to(a), 0, null, null);
            CL.clEnqueueWriteBuffer(
                    (cl_command_queue) resourceManager.getCachedData("commandQueue"),
                    bBuffer, CL.CL_TRUE, 0, size * Sizeof.cl_float,
                    Pointer.to(b), 0, null, null);
            // Set kernel arguments
            CL.clSetKernelArg(kernel, 0, Sizeof.cl_mem, Pointer.to(aBuffer));
            CL.clSetKernelArg(kernel, 1, Sizeof.cl_mem, Pointer.to(bBuffer));
            ...
            CL.clEnqueueReadBuffer(
                    (cl_command_queue) resourceManager.getCachedData("commandQueue"),
                    cBuffer, CL.CL_TRUE, 0, size * Sizeof.cl_float,
                    Pointer.to(c), 0, null, null);
            // Release buffers
            resourceManager.releaseBuffer(aBuffer);
            resourceManager.releaseBuffer(bBuffer);
            ...
            // Fall back to CPU implementation
            new CpuMatrixOperation(provider).add(a, b, c, size);
        }
    }
    public void subtract(float[] a, float[] b, float[] c, int size) {
        log.debug("OpenCL matrix subtract: {} elements", size);
        try {
            // Get kernel
            cl_kernel kernel = resourceManager.getOrCreateKernel(
            ...
            // Fall back to CPU implementation
            new CpuMatrixOperation(provider).subtract(a, b, c, size);
        }
    }
    public void scalarMultiply(float[] a, float[] result, float scalar_value, int elements) {
        log.debug("OpenCL scalar multiply: {} elements by {}", elements, scalar_value);
        try {
            // Get kernel
            cl_kernel kernel = resourceManager.getOrCreateKernel(
                    "scalarMultiply", SCALAR_MULTIPLY_KERNEL);
            // Allocate buffers
            cl_mem aBuffer = resourceManager.allocateBuffer(elements * Sizeof.cl_float, "float");
            cl_mem bBuffer = resourceManager.allocateBuffer(elements * Sizeof.cl_float, "float");
            // Copy data to device
            CL.clEnqueueWriteBuffer(
                    (cl_command_queue) resourceManager.getCachedData("commandQueue"),
            ...
            CL.clSetKernelArg(kernel, 0, Sizeof.cl_mem, Pointer.to(aBuffer));
            CL.clSetKernelArg(kernel, 1, Sizeof.cl_mem, Pointer.to(bBuffer));
            CL.clSetKernelArg(kernel, 2, Sizeof.cl_float, Pointer.to(new float[]{scalar_value}));
            ...
            // Fall back to CPU implementation
            new CpuMatrixOperation(provider).scalarMultiply(a, result, scalar_value, elements);
        }
    }
    public void transpose(float[] a, float[] b, int rows, int cols) {
        ...
        try {
            ...
            // Allocate buffers
            cl_mem aBuffer = resourceManager.allocateBuffer(rows * cols * Sizeof.cl_float, "float");
            cl_mem bBuffer = resourceManager.allocateBuffer(rows * cols * Sizeof.cl_float, "float");
            // Copy data to device
            CL.clEnqueueWriteBuffer(
                    (cl_command_queue) resourceManager.getCachedData("commandQueue"),
    ...
    public void release() {
        log.debug("Releasing OpenCL matrix operation resources");
        try {
            if (resourceManager != null) {
                // ResourceManager cleanup method doesn't exist, remove this call
            }
        } catch (Exception e) {
            log.error("Error releasing OpenCL matrix operation resources", e);
        }
    }
}
