/*
 * ID: K-001
 * Requirement: kernels.cu must define CUDA __global__ kernel implementations for matrix multiply, softmax, TF-IDF, and n-gram extraction.
 * Purpose: Core CUDA kernel file: matMulKernel (tiled SGEMM), softmaxKernel, tfidfKernel, cosineSimilarityKernel, ngramExtractKernel.
 * Rationale: Separating kernel code into a .cu file enables independent compilation, profiling, and replacement without changing the launcher layer.
