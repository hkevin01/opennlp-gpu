/*
 * ID: RO-001
 * Requirement: RocmOperations.cpp must implement the primary JNI bridge between Java RocmUtil and native ROCm/HIP GPU kernels.
 * Purpose: C++ source providing all Java_org_apache_opennlp_gpu_rocm_RocmUtil_* JNI method implementations for ROCm device init, query, matrix ops.
 * Rationale: RocmOperations is the ROCm analogue of CudaOperations; the parallel structure enables systematic feature parity across vendors.
