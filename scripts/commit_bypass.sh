...
if [ "$SYNC_REMOTE" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        # Use force push if required due to history rewrite
        if [ "${FORCE_PUSH_REQUIRED:-false}" = "true" ]; then
            echo -e "${YELLOW}⚠️  Using force push due to history rewrite...${NC}"
        ...
        if [ "${FORCE_PUSH_REQUIRED:-false}" = "true" ]; then
            echo -e "${YELLOW}⚠️  Using force push due to history rewrite...${NC}"
            git push --force-with-lease -u $REMOTE_NAME main
        else
